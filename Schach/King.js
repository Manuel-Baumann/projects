class King{
  ...
  draw(context, tileSize){
    context.fillText("K", this.x * tileSize, (this.y+1)*tileSize);
  }
  ...
  clicked(context, tileSize, whitePieces, blackPieces){
    ...
    //All possible Moves
    var possMoves = [[this.x-1, this.y-1], [this.x-1, this.y], [this.x-1, this.y+1],
     [this.x, this.y-1], [this.x, this.y+1], [this.x+1, this.y-1],
     [this.x+1, this.y], [this.x+1, this.y+1]];

    //Going through all 8 Possibilities
    //White King: If white Piece on any square -> cant move there
    if(this.isWhite){
